...
from matcher import analyze_match, analyze_sections, compare_resumes
...
# Resume selector for Tab 1 and Tab 2
...
if uploaded_files:
    ...
    if len(uploaded_files) > 1:
        selected_name = st.selectbox("Select resume for Global / Section Analysis:", file_names)
...
tab1, tab2, tab3 = st.tabs(["Global Analysis", "Section Analysis", "Compare Resumes"])
            ...
            with st.spinner(f"Comparing {len(uploaded_files)} resumes... this takes 30-60 seconds"):
                try:
                    ...
                    if rows:
                        ...
                        df["Missing Keywords"] = df["Missing Keywords"].apply(
                            lambda x: ", ".join(x))
